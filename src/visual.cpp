...
inline void showScore(int64_t _time) // 供gameOverRender调用，绘制分数
{
    ...
    int rank{0};
    if (CHARTINFO.score >= 9900000)
        rank = 0;
    else if (CHARTINFO.score >= 9800000)
        rank = 1;
    else if (CHARTINFO.score >= 9500000)
        rank = 2;
    else if (CHARTINFO.score >= 9200000)
        rank = 3;
    else if (CHARTINFO.score >= 8900000)
        rank = 4;
    else if (CHARTINFO.score >= 8600000)
        rank = 5;
    else
        rank = 6;
    ...
    if (_time > 1400)
    {
        if (CHARTINFO.tracklost)
            drawAscii(8, 2, TL, White);
        else
            drawAscii(2, 2, TC, White);
    }
    ...
    if (_time >= 3000)
    {
        ...
        {
        case 0:
            drawAscii(start_x, start_y, EXp, color);
        case 1:
            drawAscii(start_x, start_y, EX, color);
        case 2:
            drawAscii(start_x, start_y, AA, color);
        case 3:
            drawAscii(start_x + 4, start_y, A, color);
        case 4:
            drawAscii(start_x + 4, start_y, B, color);
        case 5:
            drawAscii(start_x + 4, start_y, C, color);
        case 6:
            drawAscii(start_x + 4, start_y, D, color);
        }
    }

    int x = center_x - 6;
    int y = MARGIN_T + RAIl_HEIGHT - 3;
    ...
    drawString(x, y, "Pure", Blue);
    drawString(x + 11, y, pure, White);
    drawString(x, y + 1, "Far", Yellow);
    drawString(x + 11, y + 1, _far, White);
    drawString(x, y + 2, "Lost", Red);
    drawString(x + 11, y + 2, lost, White);
    drawString(x, y + 3, "Max Recall", White);
    drawString(x + 11, y + 3, rec, White);
}
...
inline void lineRender() // 供gameOpenRender绘制谱面分割线等
{
    ...
    for (int x = MARGIN_L; x <= BORDER; x++)
        drawChar(x, MARGIN_T + RAIl_HEIGHT, '=', color); // 判定线
    for (int x = MARGIN_L; x <= BORDER; x += (RAIL_WIDTH + 1))
    {
    ...
    for (int x = MARGIN_L; x <= BORDER; x++)
        drawChar(x, MARGIN_T + RAIl_HEIGHT + 2, '-', color);
}
...
void gameOverRender() // 绘制结束时的效果
{
    ...
    audioInit();
    if (CHARTINFO.tracklost)
        audioPlay("sound/lost.wav");
    else
        audioPlay("sound/complete.wav");
    int64_t s = getNowMs();
    while (true)
    {
        int64_t now = getNowMs();
        ...
        {
            if (CHARTINFO.tracklost)
                drawAscii(MARGIN_L - 5, MARGIN_T + RAIl_HEIGHT / 2 - 4, TL, White);
            else
                drawAscii(MARGIN_L - 5, MARGIN_T + RAIl_HEIGHT / 2 - 4, TC, White);
        }
        render();
        Sleep(1);
    }
    ...
    if (CHARTINFO.tracklost)
        audioPlay("sound/tracklost.wav");
    else
        audioPlay("sound/trackcomplete.wav");
...
inline void drawChartChoice(std::string _name, int _x, int _y) // 绘制相应的谱面名称（含边框） arg: 左上角 x ,y
{
    ...
    for (int x = _x; x <= _x + width; x++)
        drawChar(x, _y, '-', White);
    drawChar(_x - 1, _y + 1, '/', White);
    drawChar(_x + width - 1, _y + 1, '/', White);
    for (int x = _x - 2; x <= _x + width - 2; x++)
        drawChar(x, _y + 2, '-', White);
...
std::vector<std::string> ChartChoiceRender() // 选曲界面->选中的谱面路径 ->vec <str> chartpath audiopath
{
    ...
    for (auto &folder : chartfolders)
    {
        ...
        std::vector<std::string> sts = getFilesInDir(path, ".mp3"); // 若有多个，取第一个
        if (!sts.empty())
            ct.soundtrack = sts[0];
        else
            ct.soundtrack = "";
    ...
    while (true)
    {
        ...
        if (IS_DOWN(VK_UP) && abs(t1 - t0) >= 100)
        {
            if (r_cursor)
            {
                ...
                // 光标已在顶部，继续向上就滚动列表（列表下移）
                else if (start_y < MARGIN_T)
                {                 // 还能继续向上滚动
                    start_y += 3; // 注意：这里是 +3，因为 start_y 越小列表越靠上
                    end_y += 3;
                    chartchange = true;
                }
            }
            // l_cursor 逻辑可以保持类似（如果你需要左右两边都支持滚动）
            if (l_cursor)
            {
        ...
        if (IS_DOWN(VK_DOWN) && abs(t1 - t0) >= 100)
        {
            if (r_cursor)
            {
                ...
                // 光标已在底部，继续向下就滚动列表（列表上移）
                else
                {
                    // 判断是否还能继续向下滚动：当前最后一个项目是否还没完全显示
                    if (end_y > VISIBLE_BOTTOM || start_y + (CHART.size() * 3) > VISIBLE_BOTTOM)
                    {
                        start_y -= 3;
                        end_y -= 3;
                        chartchange = true;
        ...
        for (auto &ct : CHART)
        {
            ...
            if (ct.y >= MARGIN_T && ct.y <= MARGIN_T + 3 * 5)
                drawChartChoice(ct.chartname, ct.x, ct.y);
            if (ct.y == r_cursorY)
            {
                choosingChart = ct;
            }
        ...
        {
        case 1:
            drawString(40, 1, "NORMAL", Yellow);
            break;
        case 0:
            drawString(40, 1, "EAZY : 增加回忆率恢复速度", Green);
            break;
        case 2:
            drawString(40, 1, "HARD : 回忆率随时间减少", Red);
            break;
        }
        ...
        for (auto &lv : choosingChart.levels)
        {
            ...
            if (l.y >= MARGIN_T && l.y <= MARGIN_T + 3 * 5)
                drawChartChoice(lv, x, l.y);
            if (l.y == l_cursorY)
            {
                choosingLevel = l;
        ...
        if ((IS_DOWN(VK_RETURN) || IS_DOWN(VK_SPACE)) && l_cursor && choosingChart.chartname != "" && choosingLevel.content != "")
        {
            goto RETURN;
        }
        if (firstloop)
...
void SettingRender() // 渲染设置界面
{
   ...
   */
    ...
    while (true)
    {
        ...
        {
            if (IS_DOWN(VK_LEFT) && DEFAULT_CHARTINFO_MODEL - 1 >= 0)
                DEFAULT_CHARTINFO_MODEL -= 1, Sleep(100);
            if (IS_DOWN(VK_RIGHT) && DEFAULT_CHARTINFO_MODEL + 1 <= 2)
                DEFAULT_CHARTINFO_MODEL += 1, Sleep(100);
        }
        switch (DEFAULT_CHARTINFO_MODEL)
        {
        case 1:
            drawString(base_x + inter, base_y, "NORMAL", Yellow);
            break;
        case 0:
            drawString(base_x + inter, base_y, "EAZY", Green);
            break;
        case 2:
            drawString(base_x + inter, base_y, "HARD", Red);
            break;
        }
        drawString(base_x + inter - 2, base_y, "<", White);
        drawString(base_x + inter + 6, base_y, ">", White);

        drawString(base_x, base_y + deltay, "谱面流速", White);
        if ((1 * deltay + base_y) == cursor_Y)
        {
            if (IS_DOWN(VK_LEFT))
                SPEEDFACTOR -= 0.2, Sleep(150);
            if (IS_DOWN(VK_RIGHT))
                SPEEDFACTOR += 0.2, Sleep(150);
        }
        std::stringstream ss;
        ...
        drawString(base_x + inter - 2, base_y + deltay, "<", White);
        drawString(base_x + inter + 6, base_y + deltay, ">", White);

        drawString(base_x, base_y + deltay * 2, "AUTOPLAY", White);
        if ((2 * deltay + base_y) == cursor_Y)
        {
            if (IS_DOWN(VK_RIGHT) || IS_DOWN(VK_LEFT))
                AUTOPLAY = !AUTOPLAY, Sleep(100);
        }
        if (AUTOPLAY)
            drawString(base_x + inter, 2 * deltay + base_y, "ON", White);
        else
            drawString(base_x + inter, 2 * deltay + base_y, "OFF", White);
        drawString(base_x + inter - 2, base_y + deltay * 2, "<", White);
        drawString(base_x + inter + 6, base_y + deltay * 2, ">", White);
