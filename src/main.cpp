...
{   
    ...
    float prcs = (CHARTINFO.proc / CHARTINFO.total_duration) * 100;
    std::stringstream ss1;
    ss1<< prcs; std::string prc = ss1.str()+ " %";
    const char *process = prc.c_str();
  
    drawString(BORDER,MARGIN_T+1,score,color);
    drawString(BORDER, MARGIN_T+3 , process,color);
...
void audioPlay(const char* _audioPath) //播放音频
{
    if (!g_audioInited) audioInit();
...
void audioPlay(const std::string & _audioPath) //播放音频
{   
    const char * path = _audioPath.c_str();
    if (!g_audioInited) audioInit();
    ma_sound_init_from_file(
        &engine,
        path,
